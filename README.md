...
Use this repository’s workflow to run all Postman collections automatically on GitHub.

1. Commit the workflow file to `.github/workflows/postman.yml`.
2. Push to `main` or open a pull request.
3. GitHub Actions will install dependencies and run:
   - `BookingApi/booking.postman_collection.json`
   - `FakeRestApi/FakeRESTApi.postman_collection.json`
   - `StudentInfoApi/StudentDetails.postman_collection.json`
4. Reports are generated in `newman-reports/` and uploaded as workflow artifacts.
...
- The workflow uses `npm ci` to install `newman` and `newman-reporter-htmlextra` from `package.json`.
- If your collection needs secrets, add them in GitHub repo settings and update the workflow to pass `--env-var` values.
